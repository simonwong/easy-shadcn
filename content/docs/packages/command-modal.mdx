...
- Unmounting a `ModalDef` or its `Provider` settles that Provider’s pending
  promises with `undefined`, without affecting sibling Providers.
- `unregister(id)` removes the global registration and settles pending
  promises for that id in every mounted Provider.
...
hook routes through context, not a module-level stack. Pending show and hide
results are isolated by Provider, even when modal ids match.
...
function show<C>(modal: C, args?: Partial<Props>): Promise<Result | undefined>
// String-id form — the id carries no type, so pass the result explicitly:
function show<T = unknown>(modal: string, args?: Record<string, unknown>): Promise<T | undefined>
```
...
// show() infers Promise<FormResult | undefined> AND validates the args.
