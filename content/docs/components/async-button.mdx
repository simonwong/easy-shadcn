...
| `loading`   | `boolean`                                                     | -       | Controlled loading state. When provided it takes precedence over the internal async loading state — `loading={false}` fully suppresses the spinner even while an async `onClick` is in flight. |
...
Supported shadcn Button props such as `variant`, `size`, `className`, `type`, and `form` are forwarded. `AsyncButton` always owns the native button element, role, busy/disabled semantics, primitive slot marker, and generated spinner structure, so `render`, `nativeButton`, conflicting raw HTML/ARIA, `role`, and `data-slot` are rejected at both type and runtime boundaries.

## Loading indicator placement

The spinner is rendered in the most context-appropriate place, in this priority order:

1. **Icon sizes (`size="icon"`, `"icon-xs"`, `"icon-sm"`, `"icon-lg"`)** — `children` (the lone icon) is replaced by the spinner.
...
- Repeat clicks are ignored while an async `onClick` is in flight — even with `loading={false}` suppressing the spinner — so a double click can't trigger a duplicate submit.
