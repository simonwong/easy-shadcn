...
- **Reserved props.** `captionLayout`, `components`, `fixedWeeks`, and `hideNavigation` are managed internally to drive the panel navigation — passing them has no effect. If you need to customize those, use the `components/ui/calendar` primitive directly.
