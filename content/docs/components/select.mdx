...
| `maxCount`       | `number`                                                        | -              | Max selectable count in `multiple` mode. At the limit, unselected items render as `disabled`; selected items stay clickable for deselection. Ignored in single mode. |
...
| `id`             | `string`                                                        | -              | Id of the focusable control (trigger button or input), so `<label htmlFor>` / the Field component can target it. |
| `aria-describedby` / `aria-invalid` | `string` / `boolean`                         | -              | Forwarded to the focusable control — wired automatically when used inside the Field component.        |
...
- **Selected-item cache.** In `serverSideFilter` mode, the component caches the full `SelectItem` of currently selected values that have appeared in a server response. Subsequent searches can return a smaller result set without dropping the chip / trigger label for selected entries. Deselecting a value releases its cached label.
- **Default filtering** is a case-insensitive `contains` match against the string form of each item: string and number labels are matched as-is, while any other `label` (e.g. JSX) falls back to matching against the item's `value`. Pass a custom `filter` when that fallback is too lossy.
- **Closing resets the search query.** Reopening a select always loads/filters the default list; the previous query is not replayed. (With a controlled `open`, this applies to closes that go through user interaction / `onOpenChange` — not to the parent silently flipping the prop.)
