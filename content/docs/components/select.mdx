...
const { stringItems, findItem, itemToStringLabel, filterFn, selectedItems } =
  useSelectItems({ items, value })
...
| `selectedItems`     | `SelectItem[]`                                    | The full `SelectItem` objects matching the current `value`. Empty array when no value.       |
...
| `value`          | `string \| string[]`                                            | -              | Controlled value. `string` in single mode, `string[]` in multi mode.                                  |
| `defaultValue`   | `string \| string[]`                                            | -              | Uncontrolled initial value.                                                                           |
| `onValueChange`  | `(value: string \| string[] \| undefined) => void`              | -              | Called when the selection changes. `undefined` indicates the value was cleared.                       |
...
| `clearable`      | `boolean`                                                       | `false`        | Show a clear button. Single-mode only.                                                                |
...
- **Single vs multi value type.** `value` is `string` in single mode and `string[]` in multi mode. The component does not enforce this with discriminated types — if you pass `multiple` you must also pass an array. `onValueChange` receives `undefined` when the value is cleared (e.g. via the clear button).
...
- **Controlled vs uncontrolled.** `value === undefined` triggers uncontrolled mode (the component manages its own state via `defaultValue`). Passing `value={undefined}` keeps the component controlled.
