...
Other safe root props (`className`, `role`, ARIA, `orientation`, and events) are forwarded to the underlying Tabs root. The Compose layer owns the generated list/panels, root element, primitive slot, orientation marker, and activation-direction state: `children`, raw HTML, `render`, `data-slot`, `data-orientation`, and `data-activation-direction` are rejected by TypeScript and stripped at runtime.
