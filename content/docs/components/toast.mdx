...
`actionButtonProps` accepts shadcn Button props except `children`, `dangerouslySetInnerHTML`, and `onClick`; those keys cannot replace Compose-owned wiring.
...
| `actionButtonProps` | shadcn Button props | Button attributes except Compose-owned `children`, `dangerouslySetInnerHTML`, and `onClick`. |
