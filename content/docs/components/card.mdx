...
Every slot accepts a class override: `headerClassName`, `titleClassName`, `descriptionClassName`, `actionClassName`, `contentClassName`, and `footerClassName`. Other safe root props (`className`, non-owned `data-*`, ARIA, event handlers, etc.) are forwarded. The Compose layer fixes `data-slot="card"`, derives `data-size` from `size`, and generates descendants around the caller-owned `children` body; raw HTML and those two data attributes are rejected by TypeScript and stripped at runtime.
