...
| `minDate`             | `Date`                                                               | -                                                | Earliest selectable day (inclusive). Disables earlier calendar days and rejects earlier typed input. |
| `maxDate`             | `Date`                                                               | -                                                | Latest selectable day (inclusive). Disables later calendar days and rejects later typed input. |
| `disabledDates`       | `Matcher \| Matcher[]`                                               | -                                                | react-day-picker matchers for unselectable days (e.g. `{ dayOfWeek: [0, 6] }`). Also enforced on typed input. |
| `id`                  | `string`                                                             | -                                                | Id applied to the trigger (button or input) for `<label htmlFor>` / Field association. |
| `name`                | `string`                                                             | -                                                | Native form name. `withInput` only — the formatted text is what gets submitted.      |
| `aria-describedby` / `aria-invalid` | `string` / `boolean`                                   | -                                                | Forwarded to the trigger — wired automatically when used inside the Field component. |
...
- **Invalid input.** When `withInput` is enabled, blurring the input with an unparseable string silently reverts to the last valid display value. Typed dates that violate `minDate` / `maxDate` / `disabledDates` are rejected the same way. There is no inline error UI by design — wrap the component with your own field state if you need validation messaging.
- **Escape cancels.** Closing the popover with Escape discards the pending typed draft instead of committing it; every other close (blur, outside click, icon button) commits a valid draft.
