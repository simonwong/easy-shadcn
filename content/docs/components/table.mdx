...
| `getCheckboxProps` | `(record, index) => Partial<TableCheckboxProps>` | — | Per-row Checkbox props (base-ui `Checkbox.Root` shape minus Table-owned state props — typically `disabled`, `aria-label`, `data-*`; see [Base UI Checkbox](https://base-ui.com/react/components/checkbox)). `disabled: true` gates the row out of "select all". |
