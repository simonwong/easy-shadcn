...
This component covers `columns` + `dataSource` + `rowKey`, selection, single-column sorting, and loading / empty / caption / row className. More complex data operations remain outside this delivery:

- **Multi-column sorting, filtering, pagination** — derive these in your parent and feed `dataSource`. Single-column sorting is built in; use `sorter: true` for externally ordered data. Combine with `@tanstack/react-table` for broader data operations.
