...
Other safe root props (`className`, `role`, ARIA, `orientation`, `loopFocus`, etc.) are forwarded to the underlying Accordion root. The Compose layer owns the generated items, root element, primitive slot, and state markers: `children`, raw HTML, `render`, `data-slot`, `data-disabled`, and `data-orientation` are rejected by TypeScript and stripped at runtime. `data-disabled` and `data-orientation` are derived from the supported root props.
