...
| `confirmProps` / `cancelProps` | `Omit<AsyncButtonProps, "children" \| "dangerouslySetInnerHTML" \| "onClick">` | - | Extra props for the default footer buttons (e.g. `{ variant: "destructive" }`). |
...
Modal owns default-footer behavior and labels. Use `onConfirm` / `onCancel` for actions and `confirmText` / `cancelText` for labels; those paths cannot be replaced through the button prop bags. Use `footer` when the whole action row must be custom. Other `AsyncButton` props remain available, including `variant`, `loading`, `disabled`, icons, `type`, `form`, and `render`.
