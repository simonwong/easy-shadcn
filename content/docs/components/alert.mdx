...
Safe root props (`id`, `style`, caller-owned `aria-*`, non-owned `data-*`, and event handlers) are forwarded to the underlying Alert root. The Compose layer fixes `role="alert"`, `data-slot="alert"`, and the child structure, so `role`, `data-slot`, `children`, and `dangerouslySetInnerHTML` are rejected by TypeScript and stripped at runtime. `title` is visible Alert content, not the native root `title` attribute.
