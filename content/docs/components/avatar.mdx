...
Safe root props such as `id`, `style`, `aria-*`, non-owned `data-*`, event handlers, `role`, and `ref` are forwarded to the underlying Avatar root. The ref resolves to its `HTMLSpanElement`. The Compose layer fixes the `span` root, generated children, `data-slot`, and the `data-size` derived from `size`; `children`, `dangerouslySetInnerHTML`, `render`, `data-slot`, and `data-size` are rejected by TypeScript and stripped at runtime.
