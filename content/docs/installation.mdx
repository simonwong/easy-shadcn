...
- **Quick try**: Install a standalone component directly via its full JSON URL. Components with internal `@easy-shadcn/*` dependencies still require the namespace mapping.
...
## Alternative: Install a Standalone Component via Full URL

You can skip the namespace config for a component whose registry dependencies are all built into shadcn:

```shell
pnpm dlx shadcn@latest add https://easy-shadcn.vercel.app/r/card.json
```

The CLI still resolves internal dependencies by their declared names. Components such as `modal`, which depends on `@easy-shadcn/async-button`, therefore need the namespace mapping even when the top-level item is addressed by full URL. The recommended namespace setup works for every component.
