...
| Date selection | Calendar, Date Picker | `Calendar`, `DatePicker` | Keep | High | DatePicker composes Calendar with input/popover state; Calendar fixes its custom-panel adapters while preserving DayPicker selection unions, root/customization props, and DayPicker's navigation authority. |
