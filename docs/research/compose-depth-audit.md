...
| [Calendar](../../registry/ui/calendar.tsx) | Adds month/year/decade view state and navigation on top of react-day-picker, including keyboard/ARIA naming. Its Interface reserves the primitive component adapters, caption layout/formatter, root marker, date library, fixed grid, and derived navigation visibility at type and runtime seams without collapsing DayPicker's selection union. Custom captions preserve primitive class/style/animation data, use DayPicker's resolved label/formatter maps, and delegate time-zone-aware mutations to DayPicker's navigation authority with reverse-aware multi-month offsets, actual-slot visibility, and month-level bounds. |
...
| [Table](../../registry/ui/table.tsx) | Owns generated table descendants, the loading-derived root busy state, row identity, selection, bulk eligibility/preservation, empty/loading output, and accessibility diagnostics. Its root rejects structure and state-marker overrides. Its row-checkbox seam fixes the Base UI element, indicator, role, selection state, derived ARIA/data, and slot marker while preserving form props, accessible naming, refs, state-aware styles, safe events, and ordinary data. The identity/selection contract is recorded in [ADR-0007](../adr/0007-table-required-rowkey-and-selection-model.md). Data Table is already this module, not another Compose. |
...
3. **Resolved:** [`Accordion`](../../registry/ui/accordion.tsx),
   ...
   intact.
4. **Resolved:** [`Table`](../../registry/ui/table.tsx) rejects generated-child
   and raw-HTML root replacement, caller-forged root busy/slot markers, and
   row-checkbox element/state-marker replacement. Disabled and readonly rows
   stay outside header bulk selection; state-aware classes, refs, safe events,
   and nullish JavaScript getter returns retain explicit contracts.
