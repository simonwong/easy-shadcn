...
| [Alert](../../registry/ui/alert.tsx) | Flattens icon/title/description/action slots. Keep thin. |
| [Avatar](../../registry/ui/avatar.tsx) | Flattens image/fallback composition. Keep thin. |
| [Breadcrumb](../../registry/ui/breadcrumb.tsx) | Generates homogeneous breadcrumb entries and separators. Keep thin. |
| [Card](../../registry/ui/card.tsx) | Flattens stable header/content/footer slots. Keep thin. |
...
| [Popover](../../registry/ui/popover.tsx) -> **Popover/Hover Card family** | Both need the same rich content slots and positioning vocabulary, so consumers should not rewrite those when changing click to link-hover preview. shadcn describes Popover as button-triggered rich portal content ([official docs](https://ui.shadcn.com/docs/components/popover)) and Hover Card as sighted-user link preview with open/close delays ([official docs](https://ui.shadcn.com/docs/components/hover-card)). | Share the task-level content/position Interface, but use the correct Popover or Hover Card primitive internally. Do not implement hover by bolting mouse events onto Popover, and do not merge Tooltip into this family. |
...
2. [ADR-0004](../adr/0004-props-vocabulary.md) still calls AlertDialog's button
   bags unsafe and Tabs' item field `label`. Current source has already fixed
   both. Update those consequence notes when the long-lived Compose policy ADR
   is written; do not treat them as active implementation debt.
...
6. Fix Modal's owned button prop bags and refresh stale ADR-0004 consequence
   notes as maintenance work.
