...
- **Completed cleanup — collection-root ownership.** Accordion, Tabs, ChoiceGroup, RadioGroup, and CheckboxGroup reject generated-child replacement and conflicting root role/orientation/state markers at type and runtime seams. ChoiceGroup keeps its four-mode value adapter stable; the soft-deprecated group entries gain no new capability.
