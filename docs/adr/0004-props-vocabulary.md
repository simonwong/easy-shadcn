...
- **Completed cleanup — form-control ownership.** InputGroup fixes its void input structure and primitive slot. Switch fixes its root element, role, generated thumb, state-derived ARIA/data attributes, size, and slot markers while preserving caller-owned naming/description ARIA, supported form/state props, styles, and events. Field fixes its generated root role/slot/orientation/state markers and flat-control id/description/invalid wiring while preserving composition children, primitive-slot escape hatches, ordinary root props, and caller control refs/events/content. All three contracts survive JavaScript or `any` callers.
...
- **Completed cleanup — Calendar interface ownership.** Calendar reserves the primitive caption layout, component adapters, root marker, date library, fixed six-week grid, derived navigation visibility, and caption formatter at both type and runtime seams. Other formatter and root customization remains caller-owned, and the type transformation distributes over DayPicker's single/multiple/range selection union instead of collapsing it. Its custom caption preserves primitive class/style/animation data and consumes DayPicker's resolved label/formatter maps with equivalent locale, numerals, and time-zone formatting options. Custom panel mutations use the same time-zone-aware date library and delegate to DayPicker's navigation authority, so caller-owned disable, controlled month, reverse-order, multi-month, and month-bound contracts remain intact.
