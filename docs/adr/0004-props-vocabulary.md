...
- **Completed cleanup — Table root ownership.** Table rejects generated-child/raw-HTML replacement plus caller-forged busy and slot markers while preserving the actual `<table>` ref, native events, accessible naming, role, styles, and ordinary ARIA/data attributes. The deeper row-selection checkbox prop bag remains a separate ownership batch.
