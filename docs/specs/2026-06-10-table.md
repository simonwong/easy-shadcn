...
- WHEN loading or empty state renders THE SYSTEM SHALL set the placeholder cell `colSpan` to `max(1, columns.length + 1)` if selectable, otherwise `max(1, columns.length)` — the floor keeps the placeholder row valid for empty `columns`.
...
- WHEN `loading` is `true` THE SYSTEM SHALL disable the header checkbox (body rows are hidden, so select-all would be a blind bulk action).
...
- [Loading / empty example](../../components/examples/table-loading-empty-demo.tsx)
- [Row className example](../../components/examples/table-row-classname-demo.tsx)
